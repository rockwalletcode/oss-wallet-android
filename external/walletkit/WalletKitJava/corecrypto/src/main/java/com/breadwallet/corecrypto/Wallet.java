...
final class Wallet implements com.breadwallet.crypto.Wallet {
    ...
    private void estimateLimit(boolean asMaximum,
        ...
        if(result.amount.getUnit().getName().equalsIgnoreCase("Ether")) {
            Optional<Double> amountAdjusted = result.amount.getDouble(result.amount.getUnit());
            if (amountAdjusted.isPresent()) {
                Double amountAdjustedDouble = amountAdjusted.get() * 0.85; // Reduce by %15 for ETH so API estimateFee succeeds
                amount = Amount.create(amountAdjustedDouble, Amount.create(result.amount).getUnit());
            } else {
        ...
        CompletionHandler<com.breadwallet.crypto.TransferFeeBasis, FeeEstimationError> estimationHandler =
            ...
            public void handleData(com.breadwallet.crypto.TransferFeeBasis feeBasis) {
                ...
                Optional<Amount> newTransferAmount = Optional.of(amount); // No need for amount.sub(newTransferFee) because the amount has already been reduced such that amount + fee < balance
